...
public class Dialog extends Element {
	...
	public Dialog(SarahsWorld game) {
	...
	public void setupWithoutAnimation(ActiveQuest quest, Thing other, String text1, String[] answers){
		...
		line = height + 20;
		for(int i = 0, y = -(Main.game().SIZE_HALF.h/2) + (answersTex.h/2) - (height + d); i < answers.length; i++, y -= height + d){
...
//		}
	...
	private void renderSpeechBubbleText(Vec shift){
		if(bubbleHeightR.v > 0.9){
			...
			for(int i = 0; i < text1.length; i++){
				font.drawString((int)(other.pos.x + shift.x - (width1[i]/2) + Render.offsetX), (float)(other.pos.y + shift.y + y - fontHeightHalf + Render.offsetY), text1[i], Color.WHITE, 1f/Main.game().SIZE_HALF.w, 1f/Main.game().SIZE_HALF.h);
				y -= line;
			}
		}
	}
	...
	private void renderAnswers(){
		...
		if(answersWidth.v > 0){
			...
			double y2 = -(Main.game().SIZE_HALF.h/2) + (answersTex.h/2);
		...
		for(int i = 0; i < answers.length; i++){
			if(answerWidths[i].v > 0){
				if(UsefulF.contains(Main.game().input2.getMousePos().x, Main.game().input2.getMousePos().y, Main.game().SIZE_HALF.w + answersTex.pixelCoords[0], Main.game().SIZE_HALF.h + ys[i], Main.game().SIZE_HALF.w + answersTex.pixelCoords[2], Main.game().SIZE_HALF.h + ys[i] + Res.menuFont.getHeight())){
				...
				font.drawString((int)(-widthsH[i]), (float)ys[i], string, Color.WHITE, 1f/Main.game().SIZE_HALF.w, 1f/Main.game().SIZE_HALF.h);
			}
		}
	}
	...
	private void drawQuadCheaty(VAO vao, Texture tex, Color color, double x1, double y1, double x2, double y2){
		Render.drawSingleQuad(vao, color, tex, x1/(x2-x1), y1/(y2-y1), (float)(x2-x1)/Main.game().SIZE_HALF.w, (float)(y2-y1)/Main.game().SIZE_HALF.h, true, 0, 1);
	}
	
	public boolean released(int button, Vec mousePos, Vec pathSincePress){
		for(int i = 0; i < answers.length; i++){
			if(UsefulF.contains(mousePos.xInt(), mousePos.yInt(), Main.game().SIZE_HALF.w + answersTex.pixelCoords[0], Main.game().SIZE_HALF.h + ys[i], Main.game().SIZE_HALF.w + answersTex.pixelCoords[2], ys[i] + Main.game().SIZE_HALF.h + Res.menuFont.getHeight())){
				other.speakPlug.say("");
				quest.onAnswer(i);
				Main.game().menu.setLast();
			}
		}
		return false;
	}
