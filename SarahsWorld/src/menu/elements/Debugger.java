...
public class Debugger extends Element {
	...
	float zero = Main.game().SIZE.h/4, sixty = zero + highValue;//vertices pixels
	int xStep = 4;//horizontal pixels
	DataSnake data = new DataSnake(Main.game().SIZE.w/5, Main.game().SIZE.w/5 * 4);
	float[] data2 = new float[Main.game().SIZE.w/5]; int dataIndex = 0;
	...
	public Debugger(SarahsWorld game) {
		...
		vao = new VAO(
				...
				new VBO(buffer, GL15.GL_DYNAMIC_DRAW, Float.BYTES, 
						new VAP(1, GL11.GL_FLOAT, false, 0)));
		zeroLine = Render.quadInScreen((short)(-2*Main.game().SIZE.w/5), (short)zero, (short)(2*Main.game().SIZE.w/5), (short)(zero+1));
		line60 = Render.quadInScreen((short)(-2*Main.game().SIZE.w/5), (short)(sixty), (short)(2*Main.game().SIZE.w/5), (short)(sixty + 1));
		background = Render.quadInScreen((short)(-2*Main.game().SIZE.w/5-220), (short)(zero-20), (short)(2*Main.game().SIZE.w/5+20), (short)(zero + 2*highValue + 21));
	}
	...
	public void render() {
		//String
		TrueTypeFont.times.drawString(20 - Main.game().SIZE_HALF.w, Main.game().SIZE_HALF.h*0.5f, fps, Color.RED, 1, 1);
		
		//Zero line
		Render.drawSingleQuad(background, new Color(0.5f, 0.5f, 0.5f, 0.5f), Texture.emptyTexture, 1f/Main.game().SIZE_HALF.w, 1f/Main.game().SIZE_HALF.h, false);
		Render.drawSingleQuad(zeroLine, Color.RED, Texture.emptyTexture, 1f/Main.game().SIZE_HALF.w, 1f/Main.game().SIZE_HALF.h, false);
		Render.drawSingleQuad(line60, Color.RED, Texture.emptyTexture, 1f/Main.game().SIZE_HALF.w, 1f/Main.game().SIZE_HALF.h, false);
		
		Shader.graph.bind();
		Shader.graph.set("scale", 1f/Main.game().SIZE_HALF.w, 1f/Main.game().SIZE_HALF.h);
		Shader.graph.set("stuff", this.highValue*4, zero, -Main.game().SIZE_HALF.w*4/5);
...
//		GL11.glEnd();
//		
//		if(Main.game().world.avatar.where.g){
//			Color.GREEN.bind();
//		} else {
//			Color.RED.bind();
//		}
