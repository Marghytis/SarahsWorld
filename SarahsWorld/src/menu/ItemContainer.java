...
public class ItemContainer extends Button {
	...
	public ItemContainer(Texture upTex, Texture downTex, Vec relativePos, int ordinal) {
		super(upTex, downTex, relativePos, () -> {Main.world.avatar.sarah.inv.selectedItem = ordinal; return true;});
	...
	public void update(double delta){
		super.update(delta);
		if(Main.world.avatar.sarah.inv.selectedItem == ordinal){
	...
	public void draw(){
		super.draw();
		ItemStack stack = Main.world.avatar.sarah.inv.stacks[ordinal];
		if(stack.item.texInv != null){
			...
			MenuManager.font.drawString(realPos.xInt() + box.size.xInt() - 10, realPos.yInt() + box.size.yInt() - 10, Main.world.avatar.sarah.life.coins + "", 0.5f, 0.5f);
			Color.WHITE.bind();
		}
	}
}
