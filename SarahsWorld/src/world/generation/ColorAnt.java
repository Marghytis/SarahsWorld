...
public class ColorAnt {
	...
	public void switchBiome(Biome newBiome) {
		...
		for(Column c = manager.lastColumn;dx >= 0 && c != null; dx--, c = manager.left? c.right() : c.left()){
			c.topColor = getTransitionColor(manager.biome.topColor, newBiome.topColor, dx, colorTransition*2);
			c.lowColor = getTransitionColor(manager.biome.lowColor, newBiome.lowColor, dx, colorTransition*2);
